...
export default function Page() {
  ...
  // When page loads, check if there are preset filters enabled in the URL!
  ...
  useEffect(() => {
    ...
    const filters = params.get('filters')?.toLowerCase().split(',') || [];
    if (filters.length === 0) return;

    const skillsArr: Skill[] = [];
    for (const x of skillsSummary) {
      x.skills.forEach((skill) => skillsArr.push(skill));
    }

    const skillsFiltered = skillsArr.filter((skill) => {
      const val = skill.value.toLowerCase();
      return filters.includes(val);
    });

    setEnabledFilters(skillsFiltered);

    // Check if search has been given
